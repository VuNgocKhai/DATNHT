...
public interface HoaDonDAO extends JpaRepository<HoaDon, UUID> {
    ...
    @Query("SELECT MONTH(ngay_thanh_toan) AS thang, SUM(tong_tien) AS tong_tien_thang\n" +
            "FROM HoaDon \n" +
            "WHERE YEAR(ngay_thanh_toan) = YEAR(CURRENT_DATE()) and trangthai =4\n" +
    ...
    @Query("SELECT SUM(tong_tien) FROM HoaDon WHERE trangthai = 4 AND ngay_thanh_toan = :ngay")
    BigDecimal tongTienTheoNgay(@Param("ngay") LocalDate ngay);

    //Tính tổng tiền doanh thu bán hàng theo tuần
    @Query("SELECT SUM(tong_tien) FROM HoaDon WHERE trangthai = 4 AND EXTRACT(WEEK FROM ngay_thanh_toan) = EXTRACT(WEEK FROM :ngay)")
    BigDecimal tongTienTheoTuan(@Param("ngay") LocalDate ngay);

    //Tính tổng tiền doanh thu bán hàng theo tháng
    @Query("SELECT SUM(tong_tien) FROM HoaDon WHERE trangthai = 4 AND YEAR(ngay_thanh_toan) = YEAR(:ngay) AND MONTH(ngay_thanh_toan) = MONTH(:ngay)")
    BigDecimal tongTienTheoThang(@Param("ngay") LocalDate ngay);

    //Tính tổng tiền doanh thu bán hàng theo năm
    @Query("SELECT SUM(tong_tien) FROM HoaDon WHERE trangthai = 4 AND YEAR(ngay_thanh_toan) = YEAR(:ngay)")
